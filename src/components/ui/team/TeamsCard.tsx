...
import { managementTeam, nutritionTeam } from "@/data/teamsCard";
...
const TeamsCard = () => {
  ...
  return (
    <div>
        ...
        {teamCategories.map((category) => (
          <ul>
            <li>
              <Button
                variant="yearly"
                className=" text-white transition duration-500 hover:bg-navbar"
              >
                {category.label}
              </Button>
            </li>
      ...
      {/* -- Nutrition Team -- */}
      <motion.section 
        initial={{opacity:0}} animate={{opacity:1, y:0}} exit={{opacity:0, y:0}} transition={{duration:1.2, ease:"linear", delay:1.8}} 
      ...
      <section className="relative grid place-content-center py-14 px-8 lg:px-24">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-44 bg-accent-var p-4 md:p-14 rounded-lg">
          <div>
            ...
            <p className="py-6">
              We are always on the lookout for talented individuals who are enthusiastic about making a difference. Explore our career opportunities and join  <br /> 
              us in our mission to help people achieve their health and wellness goals.
            </p>
          </div>
